...
# Audit Service - Development Teardown Script
# This script tears down the audit-service for development
...
show_help() {
    ...
    echo ""
    echo -e "${YELLOW}This script tears down the audit service development environment.${NC}"
    echo ""
...
if [ -f "$COMPOSE_FILE" ]; then
    ...
    if [ "$REMOVE_NETWORKS" = true ]; then
        ...
        docker network ls --filter name="${SERVICE_NAME}" -q | xargs -r docker network rm || true
    fi
